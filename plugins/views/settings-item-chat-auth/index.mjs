    ...
    render(container) {
      const channelRow = record => {
        ...
        return row(
          ...
          `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
            <span style="font-size:11.5px;color:var(--text-3)">读取其它渠道</span>${toggle('read', record.crossReadable === true, '跨渠道读取')}
            <span style="font-size:11.5px;color:var(--text-3)">发送到其它渠道</span>${toggle('send', record.crossSendable === true, '跨渠道发送')}
          </div>`,
        )
      }
      ...
      const render = () => {
        ...
        container.innerHTML = page(
          '渠道授权',
          '跨渠道读取 / 发送、手动授权记录和审计日志都集中在这里；敏感操作是否还需要二次确认由「通用 → 敏感操作确认」控制。',
          `
            ${section(
              `渠道跨渠道策略（${channelsList.length}）`,
              channelsList.length
                ? card(channelsList.map(channelRow).join(''))
                : '<div class="settings-card"><div class="setting-row"><div class="setting-main"><div class="setting-name">还没有渠道记录</div></div></div></div>',
            )}
            <div class="settings-note">隐私渠道固定单会话隔离，不能与其它渠道互读 / 互发；普通渠道可以分别开启读取 / 发送，授权记录只是兼容旧授权的补充入口。</div>
            ${section(
      ...
      const onClick = event => {
        ...
        if (toggle) {
          ...
          toast.success(next ? '已开启跨渠道权限' : '已关闭跨渠道权限')
          render()
          return
        }
