...
const CHAT_MODE_TOOL_HINTS = {
  ...
  search_memory: '按语义搜索角色的长期记忆概括（每条约 10 轮），默认 1 条；同渠道附带原文，跨渠道默认只给概括',
...
const TOOL_RULES = [
  ...
  '只有确实缺少必要上下文时才调用 read_messages（默认当前渠道，可搜索关键词 / 序号 / 时间段 / semantic 语义）；同一轮最多读取一次，尽量用关键词、limit 和时间范围缩小结果。',
  '当用户问“我们之前聊过什么”“你还记得吗”“找以前有关某件事的对话”时，优先调用 search_memory：先用一段语义描述检索相关概括，必要时用 top_summaries 多看几条比对，再展开原文。不要直接说“我没有记忆”。',
