...
- **群聊规则面板**：
  ...
  - 白名单：只允许列表里的 QQ 触发回复；
  - 仅 @ 时回复：群消息必须 @ 登录的机器人 QQ 才触发模型；关闭后 @ 仍直接回复，普通消息按回复概率触发；
  - @ 触发也受白名单限制：可单独控制是否只允许白名单 QQ 的 @ 触发；
  - 回复概率：未 @ 机器人时按概率触发模型；可单独选择是否应用白名单；
  - 回复时引用触发消息；
  - 回复时艾特触发者；
  - 未触发时也静默写入本群上下文（默认开启）；
- **群聊上下文**：本群消息无论是否触发都默认静默写入，上下文使用本群最近 **20 条消息**滚动记录，
  不会和其它私聊渠道串记忆；
- **发现会话**：连接后会拉取好友 / 群列表并记录最近消息，可在渠道详情里直接“设为目标”；
- **完整聊天记录**：入站消息按渠道落库，可在「设置 → 聊天记录」按角色 / 渠道查看与修改；
- **消息发送**：助手消息写入渠道会话后立即按顺序回复；支持文本拆分、引用、艾特与图片（`base64://` 发图）；
- **合并转发（聊天记录）**：`send_document` 发来的资料会以 QQ 合并转发发送，转发里第一条是标题、
  往下是正文（多份资料各自一条转发）；单条聊天消息超过 **`chat.forwardThreshold`**（默认 1500 字）
  也会自动折叠成转发，避免大段长文在群里刷屏。转发节点只发纯文本，模型无法借它塞 CQ 码 / 文件段；
  节点字数与节点数上限分别是 `chat.forwardNodeChars`（默认 1500）与 `chat.forwardMaxNodes`（默认 20）；
- **助手直出 CQ 码**：模型在回复正文里写 `[CQ:at,qq=123]`、`[at:123]`（简写）、`[CQ:image,file=https://…]`、
  `[CQ:face,id=14]` 等会被解析为真实消息段；`file` / `record` / `video` / `node` 等高风险类型与本地文件
  路径会被拒绝，详见下文「出站 CQ 码支持」。

## 出站 CQ 码支持

助手（模型）通过 `chat_send` 等发出的正文可以直接包含 CQ 码，NapCat 桥会在发送前解析：

| 写法 | 效果 |
|---|---|
| `[CQ:at,qq=123456789]` 或简写 `[at:123456789]` | 真实 @ 该成员 |
| `[CQ:at,qq=all]` 或简写 `[at:all]` | @全体成员（需机器人是管理员且有剩余次数） |
| `[CQ:image,file=https://…]`、`[CQ:image,file=base64://…]` | 发图（仅 http(s) / base64 / data:image） |
| `[CQ:face,id=14]` | QQ 表情 |
| `[CQ:music,type=qq,id=…]` | 音乐卡片（NapCat 支持 qq / 163 / kugou / kuwo / migu / custom） |
| `[CQ:json,data=…]`、`[CQ:xml,data=…]` | 卡片消息（data 最长 20000 字符） |
| `[CQ:reply,id=…]` | 引用指定消息 |

安全限制：`file` / `record` / `video` / `node` / `forward` 等类型会被直接丢弃；图片拒绝 `file://` 与本地路径，
防止模型通过 CQ 码让 NapCat 读取本机文件。私聊场景下 `at` 会自动降级为 `@QQ号` 文本；解析后的消息
仍遵守 3600 字符分条与 4 张图上限，引用 segment 会放在第一批消息开头。
...
- `群聊`：`channel-only` 模式，只取本群最近 20 条消息记录，未触发也静默写入；
...
- 群聊不回复：确认消息确实 @ 了登录的机器人 QQ；开启“仅 @ 时回复”时普通消息不会触发，@ 消息始终优先回复（除非勾选“@ 触发也受白名单限制”且发送者不在白名单）；黑名单会完全忽略消息。
