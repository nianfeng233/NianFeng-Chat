...
念风 Chat 的核心不是给 API 套一层聊天界面，而是把**记忆、上下文、工具、渠道和整个 UI 都拆成 cordis 插件**。
下面这些不是概念图，而是当前代码里已经存在、可以通过命令复现的能力。
...
- 长资料进入 `document-service`：单份最多 2M 字符，聊天记录里只留 `doc_id + 标题 + 摘要`；
  ...
  可以连续读到结尾。
- 因此模型看到的是“摘要 + 最近上下文 + 按需检索结果”，而不是把全部历史硬塞进 prompt。

### 2. 跨会话 / 跨渠道交互

- `chat-store.workingMessages()` 会聚合**同一角色的所有普通私聊渠道**，按时间合并去重。
  在 A 会话说过的事，切到 B 渠道后角色仍然知道，这是设计好的工作记忆，不是碰运气。
...
- 上下文构建时会把当前可用的其它渠道名称注入 system prompt，模型不需要猜内部 ID，
  也不再把跨渠道一律当作“不可用”。
