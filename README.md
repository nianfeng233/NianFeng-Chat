...
- **运行日志**：设置 → 系统 → 运行日志，终端 / `runtime.log` / WebUI 三端统一且持久化；默认显示 DEBUG 级细节，包含模型请求开始 / 完成 / 超时、工具调用耗时、权限确认、渠道外发、HTTP 请求与前端聊天链路日志；SSE 实时推送 + 轮询兜底，刷新页面或重启后端都不丢历史。
...
- **运行日志**：设置 → 系统 → 运行日志；可按级别 / 分类 / 关键词筛选，支持暂停、清空与复制，并会标红超时和失败外发。
