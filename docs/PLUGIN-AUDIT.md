...
外部插件不再被强制要求主版本与内核一致（v2.0.2 期间曾用主版本号拦截 1.x 插件，
该规则已移除）。需要表达内核兼容性时，可在插件模块或 `manifest.json` 中声明
`minAppVersion` / `maxAppVersion`；只有显式声明且当前内核不满足时才会被标记为
「内核不兼容」并阻止激活。未声明的插件按依赖、`inject` 与实际运行结果处理。
