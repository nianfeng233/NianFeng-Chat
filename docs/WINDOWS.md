...
风语在 Windows 上的目标：**双击一个脚本就能用**，不需要记命令行。
...
3. 安装完成后再双击 **`start.cmd`**，浏览器会自动打开风语
...
| `scripts/create-shortcut.ps1` | 创建桌面快捷方式 | 一键生成“风语 AI Chat”快捷方式 |
...
会在桌面生成“风语 AI Chat”，双击等价于运行 `start.cmd`。
...
关闭黑窗口（或按 Ctrl+C）即退出风语；所有数据已落盘，下次启动会恢复。
...
注意：风语运行本身**不需要**代理，只有安装依赖和调用云端模型时可能需要。
