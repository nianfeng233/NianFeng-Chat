...
| `depends` | `{ 插件名: semver范围 }`，必须依赖；缺失 / 未激活 / 加载失败 / 版本不匹配会标红并阻止加载 |
...
- **必须依赖版本不匹配**：插件列表标红，本插件不激活；页面会显示声明的范围与实际版本。
- **外部插件内核兼容性**：不要求插件版本与内核主版本一致。插件可在模块或 `manifest.json` 里用 `minAppVersion` / `maxAppVersion` 显式声明支持的内核版本范围；只有声明了且当前内核不满足时才标红并阻止激活。没有声明就按实际依赖、`inject` 与运行时错误正常处理，不再因为插件自身版本号（如 1.x）被强制拦截。
