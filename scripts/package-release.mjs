...
// extensions/ 是独立外部插件工作目录；video/ 是本地宣传片工程目录，均不随本体发布。
const SOURCE_SKIP_DIRS = new Set(['release', 'node_modules', '.git', '.tmp', '.local', '.workbuddy', 'user_data', 'data', 'extensions', 'tools', 'video'])
...
async function buildWebRelease() {
  ...
  await writeFile(
    join(deployDir, '启动念风-无浏览器.cmd'),
    [
      '@echo off',
      'chcp 65001 >nul',
      'setlocal',
      'set "NIANFENG_HOME_DIR=%~dp0"',
      'rem 服务器 / 无桌面环境：不自动打开浏览器，控制台里会打印访问地址',
      '"%~dp0runtime\\node.exe" "%~dp0app\\start.mjs" --serve --no-open',
      'if errorlevel 1 pause',
      '',
    ].join('\r\n'),
    'utf8',
  ...
  await writeFile(
    ...
    [
      '念风chat · Web 可部署版',
      '',
      '1. 有桌面的电脑：双击「启动念风.cmd」，程序会启动本地服务并自动打开默认浏览器。',
      '2. 云服务器 / Windows Server：双击「启动念风-无浏览器.cmd」，程序只启动服务，不依赖 WebView2；控制台会打印访问地址。',
      '3. 首次启动会读取本机 AppData 的数据目录指针，自动恢复上次使用的数据；之后固定使用当前目录 user_data 下的指针，不会反复覆盖数据目录。删除 user_data 可恢复全新状态。',
      '4. 默认端口 5173，如果被占用会明确提示；可通过环境变量 WEB_PORT 修改。',
      '5. 本目录自带便携 Node 运行时（runtime/node.exe），无需另装 Node.js，也不需要 WebView2。',
      '6. 内置微信clawbot 渠道插件：添加渠道菜单选择「微信clawbot」，详情点「接入」扫码即可使用。',
      '7. 内置 NapCat 渠道插件：添加渠道菜单选择「NapCat」，可填目标 QQ / 群号并复用已有 NapCat 连接。',
      '8. 外部插件：默认放在本目录 user_data\\plugins\\（也可在「设置 → 插件 → 插件目录」指定任意目录），放入插件文件夹后重新扫描/刷新即可，无需重新生成 registry.mjs。',
      '',
    ].join('\r\n'),
    'utf8',
  )
...
async function buildDesktopRelease() {
  ...
  await writeFile(
    ...
    [
      ...
      '7. 内置 QQ官方机器人 渠道插件：添加渠道选择「QQ官方机器人」，支持私聊与群聊（群内 @机器人 直接触发；群管理员开启「机器人可获取群内全部消息」后，普通群消息也会进入并按概率 / 白名单规则处理；群成员以 member_openid 标识、群昵称显示；同一「联动标识」的两个 bot 可互见并自动接话）；详情点「接入」后用手机 QQ 扫一扫二维码。',
      '8. 内置 NapCat 渠道插件：添加渠道选择「NapCat」，私聊 / 群聊 / 隐私可选；一个 NapCat 连接可被多个渠道复用。',
      '9. 内置图片服务：Web 上传 / 渠道入站图片统一存到数据目录 images/，聊天记录只存 imageId，升级不会丢。',
      '10. 外部插件：默认放在 %LOCALAPPDATA%\\NianFengChat\\user_data\\plugins\\，也可在「设置 → 插件 → 插件目录」指定任意目录；放入插件文件夹后重新扫描/刷新即可，升级 exe 不会删除外部插件。',
      '11. 系统缺少 WebView2 Runtime 时，exe 会先尝试自动下载安装器并静默安装（国内加速镜像优先；可用 NIANFENG_WEBVIEW2_INSTALLER_URL 指定镜像，或用 NIANFENG_WEBVIEW2_INSTALLER_PATH 指定本机安装器），安装成功后正常启动；安装失败才回退到默认浏览器，并继续在后台运行 Node。云服务器也可以直接用 Web 版「启动念风-无浏览器.cmd」，完全不需要 WebView2。',
      '',
    ].join('\r\n'),
    'utf8',
  )
